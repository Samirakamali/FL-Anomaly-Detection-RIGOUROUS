...
# #---------------------------------- EaaS pailliar Aggregation------------------------------------
# #------------------------------------------------------------------------------------------------

#         elif self.args.mode == 'Paillier':  
#             update_w_avg = copy.deepcopy(self.clients_update_w[0])
#             print(f"Type of update_w_avg: {type(update_w_avg)}")
#             print(f"Number of keys in update_w_avg: {len(update_w_avg)}")
#             print(f"Keys in update_w_avg: {list(update_w_avg.keys())}")

#             for k in update_w_avg.keys():
#                 print(f"Processing key (k): {k}, Type: {type(k)}")
#                 client_num = len(self.clients_update_w)

#                 for i in range(1, client_num):  
#                     for j in range(len(update_w_avg[k])):  
#                         print(f"Index (j): {j}, Type: {type(j)}")
                                                
#                         if not isinstance(update_w_avg[k][j], (float, int, paillier.EncryptedNumber)):
#                             print(f"Error: update_w_avg[{k}][{j}] is not a number, it's {type(update_w_avg[k][j])}")
#                             continue  
                        
#                         if not isinstance(self.clients_update_w[i][k][j], (float, int, paillier.EncryptedNumber)):
#                             print(f"Error: self.clients_update_w[{i}][{k}][{j}] is not a number, it's {type(self.clients_update_w[i][k][j])}")
#                             continue 
                        
#                         update_w_avg[k][j] += self.clients_update_w[i][k][j]

#                     print(f"Type of update_w_avg[{k}] after summation: {type(update_w_avg[k])}")
#                     if isinstance(update_w_avg[k], list):
#                         print(f"Length of update_w_avg[{k}] after summation: {len(update_w_avg[k])}")
#                     else:
#                         print(f"Value of update_w_avg[{k}] after summation: {update_w_avg[k]}")

#                 for j in range(len(update_w_avg[k])):
#                     update_w_avg[k][j] /= client_num  
#                     print(f"Type of update_w_avg[{k}][{j}] after averaging: {type(update_w_avg[k][j])}")
#                     print(f"Value of update_w_avg[{k}][{j}] after averaging: {update_w_avg[k][j]}")
